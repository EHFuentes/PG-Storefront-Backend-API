...
class OrdersModel {
    ...
    async addProduct(order_id, product_id, product_quantity) {
        // add product to order if order is active
        try {
            const conn = await database_1.default.connect();
            const sql = 'INSERT INTO order_product (order_id, product_id, product_quantity) VALUES($1, $2, $3) RETURNING *';
            const results = await conn.query(sql, [
                order_id,
                product_id,
                product_quantity,
